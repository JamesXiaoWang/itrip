...
public class LoginController {
     ...
     */
    @RequestMapping(value = "/weCat_login")
    public void weCat_login(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
        System.out.println("=======进入login=========>>");
        String url = "https://open.weixin.qq.com/connect/qrconnect?";
        url += "appid=wxae23616ec8ea0459";
        url += "&redirect_uri=" + URLEncoder.encode("http://d8b42659.ngrok.io/itrip-auth/api/callBackLogin", "GBK");
    ...
    public String callBackLogin(HttpServletRequest request, HttpServletResponse response)throws Exception {
        ...
        url += "appid=wxae23616ec8ea0459";
        url += "&secret=12d731aa9478dde9a0163ae54db2fbc8";
    ...
    public JSONObject httpGet(String url,HttpServletRequest  httpServletRequest) throws Exception {
        // get请求返回结果
        JSONObject jsonResult = null;
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if(null == userService.findByUsername(openid)){
                    ...
                    userService.itripWeCatCreateUser(itripUser);
                }
                //如果用户存在，则进行查询
                if(null != userService.findByUsername(openid)){
                    itripUser=userService.findByUsername(openid);
                    Dto<ItripTokenVO> dto = this.dologin(itripUser.getUserCode(),"123456",httpServletRequest);
    ...
    @RequestMapping(value = "/add_userlink.html")
    public String add_userlink() {
        return "userlink/add";
    }
}
