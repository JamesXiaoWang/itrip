...
public class UserController {
    ...
    Dto doRegister(
        ...
        System.out.println("进入了方法register方法2>>>>>>>>");
        System.out.println("name" + userVO.getUserName() + "\t"+"password" + userVO.getUserPassword() + "\t" + userVO.getUserCode());
        try {
    ...
    Dto doRegister_sms(
            @ApiParam(name="userVO",value="用户实体",required=true)
            @RequestBody ItripUserVO userVO) throws Exception {
        System.out.println("进入了方法register方法,phone>>>>>>>>");
        System.out.println("name" + userVO.getUserName() + "\t"+"password" + userVO.getUserPassword() + "\t" + userVO.getUserCode());
        try {
            ItripUser itripUser = new ItripUser();
            itripUser.setUserCode(userVO.getUserCode());
            ...
            {
                return DtoUtil.returnFail("手机已存在，注册失败", ErrorCode.AUTH_USER_ALREADY_EXISTS);
            }
        }catch (Exception e){
            e.printStackTrace();
            return DtoUtil.returnFail(e.getMessage(), ErrorCode.AUTH_UNKNOWN);
    ...
    @ApiOperation(value="用户名验证",httpMethod = "GET",
            protocols = "HTTP", produces = "application/json",
            response = Dto.class,notes="验证是否已存在该用户名")
    @RequestMapping(value="/ckusr_sms",method=RequestMethod.GET,produces= "application/json")
    public @ResponseBody
    Dto checkUser_sms(
            @ApiParam(name="name",value="被检查的用户名",defaultValue="test@bdqn.cn")
            @RequestParam String name) {
    ...
    @RequestMapping(value = "/register_phone.html")
    public String register_phone() {
        return "register/register_phone";
    }
}
